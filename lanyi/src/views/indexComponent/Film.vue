<template>
    <div class="film">
        <div class="film-nav clearfix">
            <div class="fl location">
                <van-icon class="location-icon fl" name="location-o" />
                <div class="fl">广州</div>
            </div>
            <div class="fl toggle">电影</div>
            <div class="fl search">
        ...
        <div>
            <van-tabs v-model="activeName" :border="false" color="#FF9000" title-active-color="#FF9000">
                <van-tab title="正在热映" name="in_theaters">
                    <div class="movie">
                ...
                <van-tab title="即将上映" name="coming_soon">
                    <div class="movie">
                        <div class="movie-box" ref="moviebox" @scroll="lazyLoad">
                            <div class="movie-item clearfix" ref="movieitem" @click="viewMovieDetail({name: 'movieDetail',params:{id: item.id}})" v-for="(item,index) in soonData.data.subjects" :key="index" :id="item.id">
                                <div class="fl movie-img">
                                    <img class="auto-img" :src="item.images.medium" alt="">
                                </div>
                                <div class="fl movie-info">
                                ...
                                <div class="fl buy-info">
                                    <div class="rating">{{item.rating.average}}<span>分</span></div>
                                    <div class="buy">
                                        <van-button @click.stop="buyBallot(item.id)" size="small" plain type="primary" color="#3398FC">预售</van-button>
                                    </div>
                                </div>
                            </div>
                            <p class="tip-text" v-if="!isHas">没有更多数据可加载...</p>
        ...
        methods: {
            ...
            getMovieData() {
                ...
                this.axios({
                    ...
                    msg.data.subjects.forEach(a => {

                        // 合并导演
                        a.directorsInfo = '';
                        a.directors.forEach(b => {
                            a.directorsInfo += b.name + ' / '
                        });

                        a.directorsInfo = a.directorsInfo.slice(0,-3);

                        // 合并电影类型
                        a.genresInfo = a.genres.join(' ');

                        // 合并演员
                        a.actorInfo = '';
                        a.casts.forEach(value => {
                            a.actorInfo += value.name + ' / '
                        });

                        a.actorInfo = a.actorInfo.slice(0,-3);
                    });
                    ...
                    this.$store.dispatch('movieModule/changeData',msg);
